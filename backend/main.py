...
def analyze_text(text: str = Form(...)):
    ...
    sample_texts = [p["text"] for p in COLLECTED_POSTS if p.get("text")]
    sample_embeddings = embed_model.encode(sample_texts) if sample_texts else []
    ...
    if len(sample_embeddings) > 0:
        ...
        for post, sim in zip(valid_posts, similarities):
            matches.append({
                "id": post["id"],
    ...
    relevant_matches = [m for m in matches if m["similarity"] > 0.4]
